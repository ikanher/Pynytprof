...
def _make_ascii_header(start_ns: int, header_size: int) -> bytes:
    ...
    lines = [
        ...
        "#Perl profile database. Generated by Pynytprof on " + now,
...
class Writer:
    ...
    def _write_header(self) -> None:
        ...
        static = "\n".join(lines) + "\n"
        size = len(static) + len(":header_size=00000\n") + 1

        banner = _make_ascii_header(self._start_ns, size)
...
def write(out_path: str, files, defs, calls, lines, start_ns: int, ticks_per_sec: int) -> None:
    ...
    with path.open("wb") as f:
        ...
        static_hdr = "\n".join(lines_hdr) + "\n"
        size = len(static_hdr) + len(":header_size=00000\n") + 1
        banner = _make_ascii_header(start_ns, size)
        f.write(banner)
